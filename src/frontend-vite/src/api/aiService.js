...
class AIService {
  ...
  async processInvoiceFile(file, message = "") {
    try {
      ...
      console.error("AI invoice upload error:", error);
      throw error;
    }
  }
  ...
  generateResponse(intent, extractedInfo) {
    switch (intent) {
      case "COMPARE_EXPENSES":
        return {
          response:
            "Tôi sẽ so sánh chi tiêu của bạn qua các tháng gần đây.",
