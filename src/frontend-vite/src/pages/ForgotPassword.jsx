...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Lưu resetToken để hiển thị cho người dùng (CHỈ ĐỂ TEST)
    ...
    >
      ...
      <form className={styles["forgot-form-box"]} onSubmit={handleSubmit}>
          ...
          >
            ...
            {resetToken && (
              <div className={styles["token-display"]}>
                <strong>Mã xác thực:</strong> {resetToken}
              </div>
            )}
          </div>
        )}
        ...
        <button
          ...
          disabled={isLoading}
        >
          {!isLoading && <FontAwesomeIcon icon={faEnvelope} />}
          <span className="button-text">{isLoading ? "" : "Gửi mã xác thực"}</span>
        </button>
